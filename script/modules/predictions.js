...
export const printText = (user, room) => {
  ...
  const paternName = document.querySelector(".predictions__patern_name");
  ...
  }, ((titleText.length + paternNameText.length + paternTextText.length + 10) * delay));

  setTimeout(() => {
    printTextFn(paternMottoText, paternMotto, delay);
  }, ((titleText.length + paternNameText.length + paternTextText.length + paternWishesText.length + 15) * delay));

  setTimeout(() => {
    getEndPage(room, 1, user)
  }, ((titleText.length + paternNameText.length + paternTextText.length) * delay))
}
