...
`,
  ...
  "hall": {
    ...
    predictions: ['Год будет для вас ослепительным. От ярких событий и красок иногда захочется закрыть глаза. Наслаждайтесь тем, что принесет этот год. Внимательно смотрите по сторонам, чтобы не упустить возможность обрести свое счастье.',
  ...
  'kitchen': {
    ...
    predictions: ['Год для вас будет очень доходным. К вам не только устремятся новые денежные потоки, но и поступят предложения о выгодном инвестировании. Приятными покупками станут новый автомобиль или уютная квартира.',
  ...
  "greenhouse": {
    ...
    task: `
      Соберите из кусочков этих бумажек слово. Тема, конечно же "Ботаника". Подумайте сначала немного. Если совсем не будет получаться - используйте подсказку - это кнопка в виде листочка или в виде цветочного горшка в поле ввода. Она откроет первую букву, а затем и последующие буквы. Чем больше подсказок вы будете использовать, тем больше мандарин придётся съесть
    `,
    ending: `Фух... Вы это сделали! Складываете слово на скамейке и ждёте чуда. Чудо происходит через несколько мгновений - листки закружились и полетели в другой угол двора. На глазах появилось новое растение. Вы хотели подойти поближе к нему, но как-бкдто кто-то открыл дверь на улицу настежь - стало ужасно холодно, вы торопитесь уйти со двора в ту же дверь.`,
