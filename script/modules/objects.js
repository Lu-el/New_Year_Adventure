...
`,
    task: `Если вы не в первый раз заглянули сюда и хотите продолжить своё приключение - смело нажимайте кнопку "Продолжить". Если в первый раз - выберите вариант "Новая игра". Бывалые также могут повторить эдвенчур, выбрав последний вариант.`,
    ending: `Откуда появились мандарины - вы скоро узнаете. Чем меньше вы их тратите (а значит проходите задания с максимальным результатом) - тем больше мест сможете посетить. Иногда это будет зависеть только от удачи, а иногда и от вашей логики и знаний. Как только они закончаться или вы долго будете бродить в этом месте - наступит развязка истории.`,
    mission: `
  ...
  "forest": {
    ...
    description: `<p class="book__text">Сегодня 31 декабря. Вы не выспались и настроение не очень, хотя на носу такой праздник.</p>
      ...
      <p class="book__text">Снежок прилетел вам прямо в глаз... "Ну уж нет" - подумали вы и осмотревшись вокруг увидели тропинку на снегу в лес. "Полюбуюсь зимой, а вдруг и там можно будет покаться". С этими мыслями вы отправляетесь в лес.</p>
      <p class="book__text">Уже через несколько шагов Вы видите дом, который на картинке.</p>
      <p class="book__text">"Классно оформили... Наверное турбаза какая-нибудь или дом Деда Мороза - вам стало смешно - Кажется пора возвращаться" - подумали вы. Но повернувшись, тропинки вы не увидели.</p>
      <p class="book__text">Единственным выходом было обратиться за помощью в этот наверняка жилой дом.</p>`,
    task: `А вот и первое судьбоносное решенние. Выберите вход в дом, которым вы воспользуетесь. Сначала подумайте, затем укажите в предлагаемом списке. Также необходимо будет ввести имя, с которым вы войдёте в дом(не менее 3 букв). Изменить имя и решение после внесения - будет нельзя`,
  ...
  "library": {
    ...
    description: `
      ...
      <p class="book__text">Любопытство одолело Вас и Вы решили проверить себя. Взяли один из этих тестов, нашли на столике карандаш, сели в кресло и стали отвечать на вопросы.
      </p>
    `,
    descriptionW: `
    <p class="book__text">
  ...
  "dining": {
    ...
    task: `
      Вам нужно найти пару тарелок c одинаковым рисунком, используя как можно меньше попыток. Как только вы открываете третью тарелку, остальные будут закрываться, так что не расслабляйтесь.`,
